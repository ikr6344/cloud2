...
const DevoirWidget = ({ elementModuleId }) => {
  ...
  useEffect(() => {
    const fetchDevoirs = async () => {
      try {
        const response = await axios.get(`http://localhost:3000/devoir/dev/6qZOTQbvLBWiFiIoRyPY`);
    ...
    const fetchStudents = async () => {
      try {
        const response = await axios.get(`http://localhost:3000/etudiant/elementmodule/6qZOTQbvLBWiFiIoRyPY`);
